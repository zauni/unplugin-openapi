...
import Parser3 from "@readme/openapi-parser";
import {
  createFilter,
  dataToEsm
...
var unpluginFactory = (opts) => {
  ...
  function shouldTransform(id) {
    return options.extensions?.some((ext) => id.toLowerCase().endsWith(ext)) && filter(id);
  }
  return {
    ...
    async transform(_, id) {
      ...
      if (this.addWatchFile) {
        const refs = await Parser3.resolve(id);
      ...
      const content = await Parser3.bundle(id);
      return {
