...
var require_dist = __commonJS({
  "node_modules/openapi-types/dist/index.js"(exports) {
    "use strict";
...
__export(src_exports, {
  default: () => src_default,
  getOpenAPIV2: () => getOpenAPIV2,
  getOpenAPIV3: () => getOpenAPIV3,
  isOpenAPIV2: () => isOpenAPIV2,
  isOpenAPIV3: () => isOpenAPIV3,
  unplugin: () => unplugin,
...
var _querystring = require('querystring'); var querystring = _interopRequireWildcard(_querystring);
...
var getFilename = () => _url.fileURLToPath.call(void 0, import.meta.url);
var getDirname = () => _path2.default.dirname(getFilename());
...
function normalizeAbsolutePath(path6) {
  if (_path.isAbsolute.call(void 0, path6))
    return _path.normalize.call(void 0, path6);
  else
    return path6;
}
  ...
  if (!Object.prototype.hasOwnProperty.call(map, "toUrl")) {
    Object.defineProperty(map, "toUrl", {
      ...
      value: function toUrl() {
        return `data:application/json;charset=utf-8;base64,${_buffer.Buffer.from(this.toString()).toString("base64")}`;
      }
    });
  }
  return map;
  ...
  return {
    ...
    emitFile(emittedFile) {
      ...
      if (initialOptions.outdir && emittedFile.source && outFileName) {
        const outPath = _path2.default.resolve(initialOptions.outdir, outFileName);
        const outDir = _path2.default.dirname(outPath);
        if (!_fs2.default.existsSync(outDir))
          _fs2.default.mkdirSync(outDir, { recursive: true });
        _fs2.default.writeFileSync(outPath, emittedFile.source);
      }
    },
    getWatchFiles() {
      return watchFiles;
...
function getEsbuildPlugin(factory) {
  return (userOptions) => {
            ...
            if (onTransformCb) {
              const newArgs = {
                ...
                async getContents() {
                  ...
                  return fsContentsCache = await _fs2.default.promises.readFile(args.path, "utf8");
                }
              };
    ...
    return {
      name: _nullishCoalesce((plugins.length === 1 ? plugins[0].name : meta.esbuildHostName), () => ( `unplugin-host:${plugins.map((p) => p.name).join(":")}`)),
      setup: setupPlugins
    };
  };
}
function buildSetup(meta) {
  return (plugin) => {
    return (_build) => {
      ...
      const onResolveFilter = _nullishCoalesce(((_a = plugin.esbuild) == null ? void 0 : _a.onResolveFilter), () => ( /.*/));
      const onLoadFilter = _nullishCoalesce(((_b = plugin.esbuild) == null ? void 0 : _b.onLoadFilter), () => ( /.*/));
      const loader = _nullishCoalesce(((_c = plugin.esbuild) == null ? void 0 : _c.loader), () => ( guessLoader));
      ...
      if (plugin.load) {
        onLoad({ filter: onLoadFilter }, async (args) => {
          ...
          const { errors, warnings, mixedContext } = createPluginContext(context);
          const resolveDir = _path2.default.dirname(args.path);
          let code, map;
    ...
    emitFile(emittedFile) {
      ...
      if (emittedFile.source && outFileName) {
        context.emitFile({
          ...
          name: outFileName,
          content: [..._buffer.Buffer.from(emittedFile.source)],
          resourceType: _path.extname.call(void 0, outFileName)
        });
      }
    },
    getWatchFiles() {
...
function guessIdLoader(id) {
  return ExtToLoader2[_path2.default.extname(id).toLowerCase()] || "js";
}
...
function toFarmPlugin(plugin, options) {
  ...
  if (plugin.resolveId) {
    ...
    let filters = [];
    if (options)
      filters = _nullishCoalesce((options == null ? void 0 : options.filters), () => ( []));
    farmPlugin.resolve = {
      filters: { sources: [".*", ...filters], importers: [".*"] },
      async executor(params, context) {
        const resolvedIdPath = _path2.default.resolve(
          process.cwd(),
          _nullishCoalesce(params.importer, () => ( ""))
        );
        let isEntry = false;
        ...
        const resolveIdResult = await _resolveId.call(
          ...
          params.source,
          _nullishCoalesce(resolvedIdPath, () => ( null)),
          { isEntry }
        );
  ...
  if (plugin.transform) {
    ...
    farmPlugin.transform = {
      ...
      async executor(params, context) {
        ...
        const loader = _nullishCoalesce(params.moduleType, () => ( guessIdLoader(params.resolvedPath)));
...
function createBuildContext2(compiler, compilation, loaderContext) {
  return {
    ...
    addWatchFile(file) {
      compilation.fileDependencies.add(_path.resolve.call(void 0, process.cwd(), file));
    },
    getWatchFiles() {
      ...
      if (emittedFile.source && outFileName) {
        ...
        compilation.emitAsset(
          ...
          new sources.RawSource(
            typeof emittedFile.source === "string" ? emittedFile.source : _buffer.Buffer.from(emittedFile.source)
          )
        );
      }
    }
...
function encodeVirtualModuleId(id, plugin) {
  return _path.resolve.call(void 0, plugin.__virtualModulePrefix, encodeURIComponent(id));
}
function decodeVirtualModuleId(encoded, _plugin) {
  return decodeURIComponent(_path.basename.call(void 0, encoded));
}
function isVirtualModuleId(encoded, plugin) {
  return _path.dirname.call(void 0, encoded) === plugin.__virtualModulePrefix;
}
var FakeVirtualModulesPlugin = (_class2 = class {
  constructor(plugin) {;_class2.prototype.__init.call(this);
    this.plugin = plugin;
  }
  __init() {this.name = "FakeVirtualModulesPlugin"}
  apply(compiler) {
    ...
    compiler.hooks.shutdown.tap(this.name, () => {
      if (_fs2.default.existsSync(dir)) {
        _fs2.default.rmdirSync(dir, { recursive: true });
      }
    });
  }
  async writeModule(file) {
    const path6 = encodeVirtualModuleId(file, this.plugin);
    await _fs2.default.promises.writeFile(path6, "");
    return path6;
  }
}, _class2);
var TRANSFORM_LOADER = _path.resolve.call(void 0, 
  __dirname,
  false ? "../../dist/rspack/loaders/transform.js" : "rspack/loaders/transform"
);
var LOAD_LOADER = _path.resolve.call(void 0, 
  __dirname,
...
function getRspackPlugin(factory) {
  return (userOptions) => {
    return {
      apply(compiler) {
        const VIRTUAL_MODULE_PREFIX = _path.resolve.call(void 0, _nullishCoalesce(compiler.options.context, () => ( process.cwd())), "node_modules/.virtual");
          ...
          if (plugin.resolveId) {
            ...
            compiler.hooks.compilation.tap(plugin.name, (compilation, { normalModuleFactory }) => {
              normalModuleFactory.hooks.resolve.tapPromise(plugin.name, async (resolveData) => {
                ...
                if (!_fs2.default.existsSync(resolved)) {
...
function contextOptionsFromCompilation(compilation) {
  return {
    addWatchFile(file) {
      (_nullishCoalesce(compilation.fileDependencies, () => ( compilation.compilationDependencies))).add(file);
    },
    getWatchFiles() {
      return Array.from(_nullishCoalesce(compilation.fileDependencies, () => ( compilation.compilationDependencies)));
    }
  };
}
function createBuildContext3(options, compiler, compilation, loaderContext) {
  ...
  return {
    ...
    addWatchFile(id) {
      options.addWatchFile(_path.resolve.call(void 0, _process2.default.cwd(), id));
    },
    emitFile(emittedFile) {
      const outFileName = emittedFile.fileName || emittedFile.name;
      if (emittedFile.source && outFileName) {
...
var TRANSFORM_LOADER2 = _path.resolve.call(void 0, 
  __dirname,
  false ? "../../dist/webpack/loaders/transform" : "webpack/loaders/transform"
);
var LOAD_LOADER2 = _path.resolve.call(void 0, 
  __dirname,
...
function getWebpackPlugin(factory) {
  return (userOptions) => {
    return {
      apply(compiler) {
        const VIRTUAL_MODULE_PREFIX = _path.resolve.call(void 0, _nullishCoalesce(compiler.options.context, () => ( _process2.default.cwd())), "_virtual_");
        const injected = compiler.$unpluginContext || {};
        compiler.$unpluginContext = injected;
        const meta = {
          framework: "webpack",
        ...
        for (const rawPlugin of rawPlugins) {
          ...
          if (plugin.transform) {
            compiler.options.module.rules.unshift({
              enforce: plugin.enforce,
              use(data2) {
                return transformUse(data2, plugin, TRANSFORM_LOADER2);
              }
            });
          }
          if (plugin.webpack)
...
var unpluginFactory = (opts) => {
  const options = { ...defaults, ...opts };
  const filter = _pluginutils.createFilter.call(void 0, _optionalChain([options, 'optionalAccess', _2 => _2.include]), _optionalChain([options, 'optionalAccess', _3 => _3.exclude]));
  const rootIds = /* @__PURE__ */ new Set();
  function shouldTransform(id) {
    return _optionalChain([options, 'access', _4 => _4.extensions, 'optionalAccess', _5 => _5.some, 'call', _6 => _6((ext) => id.toLowerCase().endsWith(ext))]) && filter(id);
  }
  return {
    ...
    async transform(_, id) {
      ...
      if (this.addWatchFile) {
        const refs = await _openapiparser2.default.resolve(id);
        const filteredRefs = refs.paths("file").filter((path6) => path6 !== id);
        for (const ref2 of filteredRefs) {
          this.addWatchFile(ref2);
        }
        rootIds.add(id);
      }
      const content = await _openapiparser2.default.bundle(id);
      return {
        code: _pluginutils.dataToEsm.call(void 0, content, {
          namedExports: false
        }),
        map: { mappings: "" }
    ...
    vite: {
       ...
       */
      handleHotUpdate(ctx) {
        if (shouldTransform(ctx.file) && !rootIds.has(ctx.file) && !ctx.modules.length) {
...
function getOpenAPIV3(doc) {
  return isOpenAPIV3(doc) ? doc : void 0;
}
function getOpenAPIV2(doc) {
  return isOpenAPIV2(doc) ? doc : void 0;
}
...
exports.unpluginFactory = unpluginFactory; exports.unplugin = unplugin; exports.src_default = src_default; exports.isOpenAPIV3 = isOpenAPIV3; exports.isOpenAPIV2 = isOpenAPIV2; exports.getOpenAPIV3 = getOpenAPIV3; exports.getOpenAPIV2 = getOpenAPIV2; exports.src_exports = src_exports;
